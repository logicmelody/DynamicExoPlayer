...
public final class Cache implements Closeable, Flushable {
	...
	private static final String TAG = "enginebai-cache";
	...
	public static String privateKey(Request request) {
		String key = key(request.url());
		if (isRequestAuthenticated(request)) {
			String jwt = parseJwt(request);
			if (!TextUtils.isEmpty(jwt)) {
				String userId = JWT.getUserId(jwt);
				if (!TextUtils.isEmpty(userId))
					key = key(request.url(), String.format("%s-%s", DIR_PRIVATE, userId));
			}
		}
		return key;
	}

	public static String key(HttpUrl url, String subDirectory) {
		if (!TextUtils.isEmpty(subDirectory))
			return subDirectory + "-" + ByteString.encodeUtf8(url.toString()).md5().hex();
		else
			return key(url);
	}

	/**
	 * For get() method, search private, public, root directory
	 *
	 * @param request
	 * @return
	 */
	...
	Response get(Request request) {
		...
		try {
			// if authenticated, search private first, public; otherwise, search public
			List<String> searchKeyList = new ArrayList<>();
			if (isRequestAuthenticated(request)) {
				searchKeyList.addAll(Arrays.asList(privateKey(request),
						key(request.url(), DIR_PUBLIC),
						key(request.url())));
			} else {
				searchKeyList.addAll(Arrays.asList(key(request.url(), DIR_PUBLIC),
						key(request.url())));
			}

			for (String key : searchKeyList) {
				Log.d(TAG, String.format("search key = %s, url = %s", key, request.url().toString()));
				snapshot = cache.get(key);
				if (snapshot != null) {
					Log.i(TAG, String.format("GET snapshot %s, %s", key, request.url().toString()));
					break;
				}
			}

			if (snapshot == null) {
				Log.i(TAG, String.format("GET null snapshot %s", request.url().toString()));
				return null;
			}
		}
		catch (IOException e) {
	...
	CacheRequest put(Response response) {
		...
		try {
			String cacheKey = key(response.request().url());
			CacheControl cacheControl = CacheControl.parse(response.headers);

			// if cache-control = public --> put into public dir no matter if authenticated.
			if (cacheControl.isPublic()) {
				cacheKey = key(response.request().url(), DIR_PUBLIC);
			} else if (cacheControl.isPrivate()) {
				// if request is authenticated, and response cache-control = private --> put into private dir with user ID
				if (isRequestAuthenticated(response.request())) {
					// remove "Bearer xyz"
					String jwt = parseJwt(response.request());
					String userId = JWT.getUserId(jwt);
					if (!TextUtils.isEmpty(userId))
						cacheKey = key(response.request().url(),
								String.format("%s-%s", DIR_PRIVATE, userId));
				}
			}

			editor = cache.edit(cacheKey);
			if (editor == null) {
				Log.w(TAG, String.format("PUT null %s, %s", cacheKey, response.request().url().toString()));
				return null;
			}
			Log.d(TAG, String.format("PUT %s, %s", cacheKey, response.request().url().toString()));
	...
	void remove(Request request) throws IOException {
		...
		// if authenticated, remove public/private, else public
		if (isRequestAuthenticated(request)) {
			keyList.addAll(Arrays.asList(privateKey(request), key(request.url(), DIR_PUBLIC), key(request.url())));
		} else {
			keyList.addAll(Arrays.asList(key(request.url(), DIR_PUBLIC), key(request.url())));
		}
		for (String key : keyList) {
			cache.remove(key);
			Log.w(TAG, String.format("REMOVE %s, %s", key, request.url().toString()));
		}
	}

	static boolean isRequestAuthenticated(Request request) {
		String authorizationHeader = request.header("Authorization");
		return !TextUtils.isEmpty(authorizationHeader);
	}

	static String parseJwt(Request request) {
		String authorizationHeader = request.header("Authorization");
		if (TextUtils.isEmpty(authorizationHeader))
			return "";

		// TODO: support other token types
		// remove bearer token type
		String[] lineSplit = authorizationHeader.split("\\s+");
		if (lineSplit[0].toLowerCase().trim().equals("bearer") && lineSplit.length == 2) {
			return lineSplit[1];
		} else
			return "";
	}
